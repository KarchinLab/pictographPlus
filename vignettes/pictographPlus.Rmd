...
The tool infers tumor clonal evolution from multi-region sequencing data from a single time point by modeling the uncertainty of mutation cellular fraction (MCF) in small somatic mutations (SSMs) and copy number alterations (CNAs). Using a Bayesian hierarchical model, it assigns SSMs and CNAs to subclones, reconstructing tumor evolutionary trees that adhere to principles of lineage precedence and sum condition. For high purity tumor samples sequenced at high coverage, optional constraints based on sample presence may be beneficial. For deconvolution, PICTographPlus integrates tumor clonal tree structures with clone proportions across samples to resolve bulk gene expression data. It optimizes an objective function that minimizes discrepancies between observed and predicted sample-level gene expression while imposing a smoothness penalty, ensuring that closely related clones display greater gene expression similarity. Lastly, the tool conducts pathway enrichment analysis to identify statistically significant alterations in pathways connecting tumor clones. PictographPlus was designed for modeling natural disease history of untreated samples, but has yielded useful results on some treated cases (Rineheart et al Nat. Prec. Onc. 2024). 
 
------

# 2. Input data file format

## 2.1 Input DNA data for tumor evolution reconstruction

PICTographPlus takes input data in multiple formats for flexible user inputs. You can input either:

1) Three csv files, one each for SSMs, CNAs, and germline heterozygous single nucleotide variant (SNVs) (see example in section 2.1.1).
2) Two csv files, one for SSM and one for CNA (see example in section 2.1.2).
3) A single csv file that contains SSM and CNA information (see example in section 2.1.3).
...
The SSM file should contain at least sample, mutation, total_reads, alt_reads, chrom, start, and end columns. Tumor purity values can be entered but are not required. If no purity values are entered, PictographPlus will estimate purity. Genomic coordinates should be provided in the 1-based, inclusive coordinate system.
...
```
The CNA file should contain sample, chrom, start, end, and tcn columns. The total copy number (tcn) can be inferred from many copy number callers. Genomic coordinates should be provided in the 1-based, inclusive coordinate system. We have had success with **CNVKit** (Talevic et al. PLOS Comp Bio 2016) and **FACETS** (Shen et al. Nuc. Acids Rec 2016). However, any copy number calling that provides TCN should work. In case a copy number caller outputs the $log2$ ratio of a segment, the tcn can be calculated using $tcn = 2 * 2^{\log_2 R}$. 

```{r}
head(read.csv(system.file('extdata/examples/example4_cna.csv', 
                          package = 'pictographPlus')))
```
The SNV file contains the count of the heterozygous germline SNVs that have the information about the "chrom", the "position" of the germline heterozygous SNV, "ref" and "alt" allele, and the reference and altenative reads counts in germline (normal) sample as well as all other samples. 

> **Note:** The sample name (`sample1`, `sample2`, ... etc.) should matched the sample name used in the SSM and CNA file. 
...
For example, if using facets for copy number calls, users can get tcn by using the formula $tcn = 2^{cnlr.median.clust}$, and baf using the formula $baf = 1 / (1 + 1 / 2 ^ {mafR.clust})$
...
### 2.1.3 Common formatting issues that may lead to errors

- **Duplicated mutation names** within the same sample will cause errors.  
- **Sample names** are limited to 12 characters to ensure optimal visualization in the output figures. In addition, all sample names must be consistent across all input files.
- **Sample name consistency:** All sample names in the CNA files must match those in the SSM files. Any extra samples present in the CNA files but missing from the SSM files will result in an error.
...
| selectedMutFile | list of genes or mutations used for visualization | NULL | runPictograph |
...
| sampleorderFile | a csv file that defines the order of samples in the piechart | NULL | runPictograph |
...
## 6.4 **SelectedMutFile**
You can annotate the edges of the tree plot generated by PictographPlus by pre-selecting the mutations or copy-number-altered gene regions that are inferred to occur on those edges. To pre-select, use the **runPictograph** parameter **selectedMutFile** to input a plain text file that lists the mutations or copy number alterations of interest and their chromosome positions in 1-based, inclusive format. We suggest you choose a naming convention that will be easy to read and interpret on your tree.

| name_on_tree | chr | start | end |
|:-----|------:|------:|------:|
| mut1 | chr1  |  100 | 100 |
| mut2 | chr2  |  500 | 500 |
| MYC AMP | chr8 | 128,747,000 |128,820,000 |



## 6.5 **sampleorderFile**
A csv file that defines the order of samples in the piechart, plotting by `runPictograph()`.
